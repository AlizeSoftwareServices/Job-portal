...
let MailService = class MailService {
    ...
    async sendApplicationConfirmation(email, firstName, jobTitle, referenceNumber) {
    ...
    async sendForgotPasswordOtpEmail(email, code) {
        if (!process.env.GMAIL_USER || process.env.GMAIL_USER === 'your-email@gmail.com') {
            console.log(`[MOCK EMAIL] OTP: ${code} to ${email}`);
            return;
        }
    ...
    async sendStatusUpdateEmail(email, firstName, jobTitle, status, referenceNumber) {
        if (!process.env.GMAIL_USER || process.env.GMAIL_USER === 'your-email@gmail.com') {
    ...
    async sendAdminNewApplicationEmail(adminEmail, candidateDetails, jobTitle, referenceNumber) {
        if (!process.env.GMAIL_USER || process.env.GMAIL_USER === 'your-email@gmail.com')
            return;
        try {
            await this.transporter.sendMail({
    ...
    async sendAdminNewEmployerEmail(adminEmail, companyName, hrName, email) {
        if (!process.env.GMAIL_USER || process.env.GMAIL_USER === 'your-email@gmail.com')
            return;
        try {
            await this.transporter.sendMail({
    ...
    async sendAdminNewJobRequestEmail(adminEmail, jobTitle, companyName) {
        if (!process.env.GMAIL_USER || process.env.GMAIL_USER === 'your-email@gmail.com')
            return;
        try {
            await this.transporter.sendMail({

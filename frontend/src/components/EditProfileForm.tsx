...
export default function EditProfileForm({ profile, onSaved, onLogout }: { profile: any, onSaved: () => void, onLogout: () => void }) {
  ...
  const handleResumeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:3000/users/profile/resume', {
        method: 'POST',
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const token = localStorage.getItem('skyo_token');
    try {
      const res = await fetch('http://localhost:3000/users/profile/avatar', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: data,
      });
        ...
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          {form.resumeUrl && (
            <div className="w-full md:w-1/3 bg-blue-50 border border-blue-100 rounded-2xl p-6 text-center flex flex-col items-center justify-center">
              ...
              <a href={`http://localhost:3000${form.resumeUrl}`} target="_blank" rel="noreferrer" className="text-sm text-blue-800 hover:underline font-medium">View Current Resume</a>
            </div>
          )}
        </div>
      </div>
